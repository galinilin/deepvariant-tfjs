...
/**
 * Force-pick a candidate at `position` regardless of DV thresholds. Used by
 * the "Predict anyway" affordance: when our threshold-aware engine returns
 * a rejected outcome (below-count / below-fraction / no-alt-evidence), the
 * user can still choose to feed something to the model and see what it says.
 *
 * Behavior:
 *   - Walks reads with the same mapq/baseQ filters as deriveCandidateOutcome.
 *   - Counts SNV / del / ins evidence; picks the highest-count alt across
 *     all kinds with the same indel-preferred tie-break.
 *   - If qualifying reads = 0 (no-coverage), returns null — there's nothing
 *     to predict on at this position.
 *   - If there is coverage but zero alt evidence, fabricates a placeholder
 *     SNV using the most common transition (A↔G, C↔T) so the model gets a
 *     non-trivial alt to evaluate. The reported supportingReads will be 0.
 */
export function forceCandidateAt(
  reads: Read[],
  reference: Base[],
  position: number,
): CandidateInfo | null {
  ...
  if (qualifyingTotal === 0) return null; // no-coverage

  // If any alt evidence, pick the best like deriveCandidateOutcome does.
  const candidates: BestAlt[] = [];
  for (const [b, c] of snvCounts) candidates.push({ kind: 'snv', altBase: b, count: c });
  if (delCount > 0) candidates.push({ kind: 'del', count: delCount });
  for (const [seqKey, c] of insCounts) {
    candidates.push({
      kind: 'ins',
      altSequence: seqKey.split('') as Base[],
  ...
  // No alt evidence — fabricate a transition (A↔G, C↔T) as the most
  // biologically plausible "what if" alt. Supporting reads is 0 so the user
  // sees that this is a fully synthetic forced prediction.
  const transition: Record<Base, Base> = { A: 'G', G: 'A', C: 'T', T: 'C', N: 'A' };
  return {
    kind: 'snv',
    refBase,
